...
namespace EbestTradeBot.Client.Services.OpenApi
{
        ...
        ) : IOpenApiService
    {
        ...
        {
            if(stock.평단가 < 0 || t1305s.Count < 2) return;

            var nowBlock = t1305s[0];
            var preBlock = t1305s[1];

            stock.익절가 = int.MinValue;
            stock.손절가 = int.MinValue;
            stock.매수가_2차 = int.MinValue;

            stock.익절가 = (int)(stock.평단가 * 1.03);
            stock.손절가 = preBlock.Close;
            stock.매수가_2차 = (stock.평단가 + stock.손절가) / 2;
            if ( stock.익절가 == int.MinValue || stock.손절가 == int.MinValue || stock.매수가_2차 == int.MinValue)
            {
                stock.매수가_2차 = int.MinValue;
                stock.익절가 = int.MinValue;
                stock.손절가 = int.MinValue;
